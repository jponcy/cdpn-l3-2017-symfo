...
/usr/bin/gitstats ./ ./doc/stats/html1/ > /dev/null
#/usr/local/bin/git_stats ./ ./doc/stats/html2/ > /dev/null

echo "Fetch Gravatar image of commiters..."
./gravatar.pl > /dev/null

echo "Generate Gource video..."
/usr/bin/gource --title "ookee web" --stop-at-end --auto-skip-seconds 1 --disable-auto-skip --max-files 0 --date-format "%d %b %y" --hide usernames,dirnames,mouse --seconds-per-day .25 --file-filter 'libs/|doc/' -user-image-dir .git/avatar/ -1280x720 -o - | ffmpeg -y -r 60 -f image2pipe -vcodec ppm -i - -vcodec libvpx -b 10000K doc/stats/lifetime.webm > /dev/null
